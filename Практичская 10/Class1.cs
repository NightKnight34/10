...
namespace allUsers
{
    ...
    {
        private static Stream jsondadta;
        ...
        public static void Create<T>(T item, string dbname)
        {
            ...
            if (File.Exists(path + "/db/" + dbname))
            {
                ...
                var allData = JsonSerializer.Deserialize<List<T>>(jsondata);
                allData.Add(item);
                //Console.WriteLine(allData[0].id);
                string savedata = JsonSerializer.Serialize(allData);
            ...
            else
            {
                ...
                newAllData.Add(item);
                string savenew = JsonSerializer.Serialize(newAllData);
        ...
        {
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
            List<T> items = JsonSerializer.Deserialize<List<T>>(jsondadta);
        ...
        public static void Update<T>(string dbname, int id, string attr, object nvalue)
        {
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
            string jsondata = File.ReadAllText(path + "/db/" + dbname);
            List<T> allData = JsonSerializer.Deserialize<List<T>>(jsondata);
        ...
        public static void Delete<T>(string dbname, int id)
        {
            ...
            List<T> allData = JsonSerializer.Deserialize<List<T>>(json);
            foreach (var d in allData)
            {
